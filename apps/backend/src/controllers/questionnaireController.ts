...
import { db } from '../db/index'; // Ensure this path is correct based on your project structure
  ...
  try {
    ...
    */
    ...
    try {
      ...
      // const [savedQuestionnaire] = await
      //   .returning();
      return reply.status(201).send(savedQuestionnaire);
    }
    catch (error) {
      request.log.error(error, 'Database error while saving questionnaire');
