...
const dailyReportBodySchema = z.object({
  date: z
    .string()
    .regex(/^\d{2}\/\d{2}\/\d{4}$/, "Date must be in dd/mm/yyyy format"), // dd/mm/yyyy
...
export const saveDailyReportHandler = async (
  ...
  try {
    ...
    const { date, gratitude, goal, motivation, mood, influence, habits } =
      parsedBody.data;
    ...
    if (existingReport.length > 0) {
      return reply
        .status(409)
        .send({ message: `A daily report for ${date} already exists.` });
    }
